...
// console.log('expression')
  ...
  const exclainmThree = str => str.toUpperCase() + '!!!'
...
/* 
  Create a function called 'bigOrSmall' that takes in one parameter, 'arr',  which will be an array of 
  numbers. 
  Inside of the bigOrSmall function, create a new array called 'answers'. 
  Then, loop over the passed in arr parameter, and check to see if the number in the array is GREATER than 
  100. 
...
function frogPurchases(gold) {
  return gold / 3
}
let totalFrogs = frogPurchases(90)
...
/*
  ...
  to you 1.3333... However, you can't really go to a store and by 1.333 products. 
  You would just be able to purchase 1 product. Re-write the function you used in the previous problem 
  (give it the same name, just add a 2 to the end of it) that fixes this bug. Invoke the function and store 
...
/*
  ...
  in ascending order. The function should return true, if it is sorted in ascending order, false if it is 
  not. Create a variable, `arrayIsAscending` and set it equal to your function invoked. Use the sample 
  array to test this function.
*/

//CODE HERE

function checkAscending(arr){
  // set starting value of first index in array
  let comparisonValue = arr[0]

  // loop over array
    for(let i = 0; i < arr.length -1; i++){

      // check to see if the next index in the array is larger than or equal to the current comparison value
      if(comparisonValue <= arr[i + 1]){
        // if it is, update the comparison varible to be equal to the next index in the array
        comparisonValue = arr[i + 1]
      } else {
        // if the next array value is LESS than the current value, break the loop and return false
        return false
      }
    }

  // if the entire loop completes without ever triggering the else portion, we know the whole array was in ascending order, and can just return true
  return true
}
